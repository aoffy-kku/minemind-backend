...
def model(id, clm, birthDate, start, end):
    """
    ...
    df = pd.concat([stat, sleep_score, sleepdetail, bop_prop], axis=1)

    selected_features = ['ad_kurt_HR(bpm)', 'ad_sd_HR(bpm)', 'ad_skew_HR(bpm)', 'mean_REM', 'ns_skew_HR(bpm)',
                         'sd_sleep_session', 'skew_sleep_session', 'start_day(%)', 'REM_20_25(%)', 'REM_Gt25(%)',
                         'REM_Lt20(%)', 'SS58(%)', 'SSLT3(%)', 'bca', 'cba', 'abc', 'cca', 'acc']
    df = df[selected_features]

    # ===== load model and predict ===== #
    clf = joblib.load('./minemind_analysis/model/model_watchV1_rf.joblib')
    # y_pred = int(clf.predict(df)[0])
    # score = max(np.round(clf.predict_proba(df) * 100, 2)[0])
